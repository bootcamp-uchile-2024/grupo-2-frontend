                ...
                cart.length === 0 ? <span className='text-neutral-800 font-xl text-center'>No hay artículos en el carrito</span>
                  : <div className='flex flex-col justify-between h-full'>
                    <div className='overflow-scroll max-h-[800px]'>
                      {
                        cart.map((item) => (
                          <div key={item.id} className='flex gap-x-4 my-6'>
                            <img src={item.imagen} alt={item.nombre} className='max-w-42 max-h-32 h-full w-full object-cover' />
                            <aside className='flex flex-col justify-center w-full'>
                              <span className='text-xl font-light'>{item.nombre}</span>
                              <span className='text-lg font-medium'>${Number(item.precio).toLocaleString()}</span>
                              <span className='text-xs font-light'>Cantidad: {item.stock}</span>
                    ...
                    <div className='flex flex-col gap-y-2'>
                      <span className='text-end text-lg'>Total: ${totalAmount ? totalAmount.toLocaleString() : '0'}</span>
