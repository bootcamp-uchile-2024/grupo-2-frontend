...
interface CartContextType {
    cart: CartType[]
    addItemToCart: (item: CervezaType) => void;
    deleteItemFromCart: (item: CervezaType) => void;
    clearCart: () => void;
    totalAmount: number
    totalItems: number
}

type CartType = CervezaType
...
export default function CartProvider({ children }: { children: React.ReactNode }) {
    ...
    function addItemToCart(item: CervezaType) {
        ...
        if (itemToFind === -1) {
            ...
            const updatedCart = cart.map(cartItem => {
                if (cartItem.id === item.id) {
                    return { ...cartItem, quantity: cartItem.stock && cartItem.stock + 1 }
                }
                return cartItem;
            })
            setCart(updatedCart);
    ...
    function deleteItemFromCart(item: CervezaType) {
